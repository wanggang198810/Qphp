...
class Upload{
    ...
    public function save($name){
        ...
        $num = count($file['tmp_name']) - 1;
        for($i=0; $i < $num ; $i++){
            $newfilename = $this->getPath() . 'ori/';
            $newfilename .= $this->getNewFilename($file['ext'][$i]);

            //检测图片
            if(!$this->checkExt($file['ext'][$i])){
                return array('success' => -1, 'result' => $this->errorCode(-1));
            }
            if(!$this->checkSize($file['ext'][$i])){
                return array('success' => -2, 'result' => $this->errorCode(-2));
            }

            $this->checkPath($newfilename);
            $ext = $this->getExt($file['name'][$i]);
